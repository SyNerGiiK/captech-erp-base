...
# ✅ Imports corrigés : get_current_user vient d'app.routers.auth
try:
    from app.routers.auth import get_current_user  # FastAPI dependency
except Exception:
...
async def get_invoice_by_id(invoice_id: int, user: dict = Depends(get_current_user)):
    itbl = models.Invoice.__table__
    q = select(itbl).where(and_(itbl.c.id == invoice_id, itbl.c.company_id == user["company_id"]))
...
# ======== Téléchargement public (via token) ========
...
async def public_download_invoice_pdf(
    ...
    pdf_bytes = await _build_invoice_pdf(invoice_id, int(data["company_id"]), vat_percent)
    fname = f"invoice_{invoice_id}.pdf"
    return StreamingResponse(
        iter([pdf_bytes]),
        media_type="application/pdf",
