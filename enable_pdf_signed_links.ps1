# file: enable_pdf_signed_links.ps1
# Usage:
#   powershell -ExecutionPolicy Bypass -File .\enable_pdf_signed_links.ps1
...
function Write-Utf8($Path,[string]$Content){ $enc = New-Object System.Text.UTF8Encoding($false); [IO.File]::WriteAllText($Path,$Content,$enc) }

# --- 1) requirements.txt : ajouter weasyprint ---
$req = Join-Path $root "backend/requirements.txt"
if(-not(Test-Path $req)){ throw "Manque: backend/requirements.txt" }
$reqLines = (Get-Content $req)
if(-not ($reqLines -match '^\s*weasyprint(\s*==.*)?\s*$')){
  $reqLines += 'weasyprint'
  Write-Utf8 $req ($reqLines -join "`n")
  Write-Host "[OK] Ajouté: weasyprint"
} else { Write-Host "[OK] weasyprint déjà présent" }

# --- 2) Dockerfile : libs système WeasyPrint ---
$df = Join-Path $root "backend/Dockerfile"
if(-not(Test-Path $df)){ throw "Manque: backend/Dockerfile" }
$dockerSrc = Read-All $df

# On insère une étape apt-get avant pip install (si absente)
if($dockerSrc -notmatch 'apt-get\s+install\b.*libcairo2'){
  $dockerSrc = $dockerSrc -replace 'COPY\s+requirements\.txt\s+\.\s*\r?\n\s*RUN\s+pip install[^\r\n]+',
@'
COPY requirements.txt .
# WeasyPrint deps (Debian)
...
'@
  Write-Host "[OK] Dockerfile patché (libs WeasyPrint)"
} else {
  Write-Host "[OK] Dockerfile déjà prêt pour WeasyPrint"
}
Write-Utf8 $df $dockerSrc

# --- 3) link_utils.py (JWT courte durée) ---
...
# --- 4) invoices.py (HTML + PDF + liens signés) ---
...
# ---- CRUD / LINES / FROM QUOTE / STATUS / SUMMARY / EXPORT CSV ----
...
async def create_signed_download_link(
    ...
    await _owned_invoice(invoice_id, user["company_id"])
    token_kind = "invoice_pdf" if kind == "pdf" else "invoice_html"
    token = create_signed_token(
        kind=token_kind,
        data={"invoice_id": int(invoice_id), "company_id": int(user["company_id"])},
        ttl_seconds=int(ttl),
    )
    path = f"/invoices/public/{invoice_id}/download.{kind}?token={token}"
    return {"path": path, "ttl": ttl, "kind": kind}

@router.get("/public/{invoice_id}/download.html")
async def public_download_invoice_html(invoice_id: int, token: str, vat_percent: int = Query(20, ge=0, le=100)):
    try:
...
async def public_download_invoice_pdf(invoice_id: int, token: str, vat_percent: int = Query(20, ge=0, le=100)):
    ...
    company_id = int(data.get("company_id",-1))
    html = await _render_invoice_html(invoice_id, company_id, vat_percent)
    # base_url requis pour les assets (polices système)
    base_dir = os.path.dirname(os.path.dirname(__file__))  # app/
...
'@
Write-Utf8 "$root/backend/app/routers/invoices.py" $invoicesPy
Write-Host "[OK] backend/app/routers/invoices.py (PDF + liens signés)"

# --- 5) Frontend : bouton “Télécharger PDF” (via lien signé) ---
$invJs = Join-Path $root "frontend/src/Invoices.jsx"
if(-not(Test-Path $invJs)){ throw "Introuvable: $invJs" }
$src = Read-All $invJs

# Retirer un éventuel ancien bouton PDF
$src = [regex]::Replace($src, '(?is)<button\b[^>]*>\s*T[ée]l[ée]charger\s*\(PDF\)\s*</button>', '', 1)

# Trouver la barre d’actions (à côté de l’input TVA et du bouton HTML)
$anchor = 'style={{ marginLeft:"auto", display:"flex", gap:8 }}'
$idx = $src.IndexOf($anchor)
if($idx -lt 0){ throw "Impossible de trouver la barre d’actions (anchor non trouvée)" }
...
<button onClick={async () => {
  ...
  try {
    ...
    const resp = await fetch(base + "/invoices/" + sel.id + "/signed_link?kind=pdf&ttl=300", { method: "POST", headers: { "Authorization": "Bearer " + token }});
    ...
    alert("Lien PDF impossible: " + e.message);
  }
}}>Télécharger (PDF)</button>
'@

$src = $src.Insert($after, "`n          " + $buttonPdf + "`n")
Write-Utf8 $invJs $src
Write-Host "[OK] frontend/src/Invoices.jsx (bouton PDF signé)"
...
Write-Host "`n✅ PDF signé activé."
Write-Host "UI: Front → Factures → 'Télécharger (PDF)' (pas d’Authorization dans l’onglet)."
Write-Host "API direct:"
Write-Host "  1) POST /invoices/{id}/signed_link?kind=pdf (avec Authorization)"
Write-Host "  2) GET  /invoices/public/{id}/download.pdf?token=...."
